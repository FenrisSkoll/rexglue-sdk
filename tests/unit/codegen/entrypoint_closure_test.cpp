...
TEST_CASE("entrypoint closure reports never mutate an unrelated manifest",
  ...
  JumpTableBoundCandidateEvidence inheritedDomain;
  inheritedDomain.compareAddress = kTextBase + 0x04;
  inheritedDomain.guardAddress = kTextBase + 0x08;
  inheritedDomain.domainOriginAddress = kTextBase + 0x20;
  inheritedDomain.value = 2;
  inheritedDomain.caseCount = 3;
  inheritedDomain.indexRegister = 7;
  inheritedDomain.inclusive = true;
  inheritedDomain.finiteDenseDomain = true;
  inheritedDomain.inheritedCaseEdgeProof = true;
  inheritedDomain.inheritedFiniteCaseDomain = true;
  inheritedDomain.finiteValues = {1, 2, 3};
  inheritedDomain.normalizedFiniteValues = {0, 1, 2};
  inheritedDomain.inheritedCaseEdges = {{kTextBase + 0x20, kTextBase + 0x30}};
  inheritedDomain.stackSpillAddress = kTextBase + 0x04;
  inheritedDomain.stackReloadAddress = kTextBase + 0x0C;
  ...
  }
  {
    ...
    CHECK(inheritedDomain.at("normalized_finite_values") == nlohmann::json::array({0, 1, 2}));
    CHECK(inheritedDomain.at("inherited_case_edges").front().at("source") == "0x10000020");
    CHECK(inheritedDomain.at("inherited_case_edges").front().at("target") == "0x10000030");
    CHECK(inheritedDomain.at("stack_spill_address") == "0x10000004");
